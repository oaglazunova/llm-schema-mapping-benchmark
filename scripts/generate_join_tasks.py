...
from lsmbench.generation.join_task_generator import build_join_specs


def _task_path(task_id: str) -> Path:
    return REPO_ROOT / "benchmark" / "tasks" / "synthetic" / "join" / f"{task_id}_task.json"
...
def build_tasks() -> list[str]:
    ...
    for spec in specs:
        ...
        task_refs.append(f"benchmark/tasks/synthetic/join/{task_id}_task.json")
...
def build_task_set(task_refs: list[str]) -> None:
    ...
    write_json(REPO_ROOT / "benchmark" / "task_sets" / "synthetic_join_v1.json", out)


def main() -> int:
    task_refs = build_tasks()
    build_task_set(task_refs)
    print(f"Wrote {len(task_refs)} join tasks.")
