...
def _task_path(task_id: str) -> Path:
    return REPO_ROOT / "benchmark" / "tasks" / "synthetic" / "copy_rename" / f"{task_id}_task.json"
...
def build_tasks() -> list[str]:
    ...
    for spec in specs:
        ...
        task_refs.append(f"benchmark/tasks/synthetic/copy_rename/{task_id}_task.json")
...
def build_task_set(task_refs: list[str]) -> None:
    out = {
        "name": "synthetic_copy_rename_v1",
        "version": "0.1.0",
        "tasks": task_refs,
        "notes": [
            "Primitive-first synthetic benchmark slice.",
            "Only copy/rename tasks with lexical perturbations.",
        ],
    }
    write_json(REPO_ROOT / "benchmark" / "task_sets" / "synthetic_copy_rename_v1.json", out)


def main() -> int:
    task_refs = build_tasks()
    build_task_set(task_refs)
    print(f"Wrote {len(task_refs)} copy/rename tasks.")
